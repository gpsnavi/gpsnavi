...
#define	CC_GEMCOMMENTALT_SEND_BODY_SIZE				3072
...
// GEMコメント変更XMLパーサ
typedef struct _GEMCOMMENTALT_PARSER {
	INT32			state;
	Char			*buf;
	GEMINFO			gemInfo;
} GEMCOMMENTALT_PARSER;

// gemcomment/alt/
enum GemCommentAltStatus {
	CC_GEMCOMMENTALT_NODE_NONE = 0,
	CC_GEMCOMMENTALT_NODE_XML,
	CC_GEMCOMMENTALT_NODE_XML_CHILD,
	CC_GEMCOMMENTALT_NODE_API_STATUS
};
...
// 関数定義
//------------------------------------------------
static void CC_GemCommentAlt_CreateUri(const T_CC_CMN_SMS_API_PRM *param, Char *uri);
static E_SC_RESULT CC_GemCommentAlt_CreateBody(const T_CC_CMN_SMS_API_PRM *parm, const Char *gemId, INT32 commentId, const Char *comment, Char *body);
static E_SC_RESULT CC_GemCommentAlt_AnalyzeHttpResp(const Char *body, E_CONTEXT_TYPE contextType, SMCALOPT *opt, Char *apiStatus);
static E_SC_RESULT CC_GemCommentAlt_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, SMCALOPT *opt);
static void XMLCALL CC_GemCommentAlt_StartElement(void *userData, const char *name, const char **atts);
static void XMLCALL CC_GemCommentAlt_EndElement(void *userData, const char *name);
static void XMLCALL CC_GemCommentAlt_CharacterData(void *userData, const XML_Char *data, INT32 len);
 ...
 */
E_SC_RESULT CC_GemCommentAlt_SendRecv(SMCAL *smcal,
									  ...
									  Char *apiStatus)
{
	...
	do {
		...
		*apiStatus = EOS;
		...
		data = (Char*)SCC_MALLOC(CC_GEMCOMMENTALT_SEND_BODY_SIZE);
		if (NULL == data) {
		...
		// URI生成
		CC_GemCommentAlt_CreateUri(parm, uri);

		// body生成
		ret = CC_GemCommentAlt_CreateBody(parm, gemId, commentId, comment, data);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemCommentAlt_CreateBody error, " HERE);
			break;
		}
		...
		// レスポンス解析
		ret = CC_GemCommentAlt_AnalyzeHttpResp(body, contextType, &opt, apiStatus);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemCommentAlt_AnalyzeHttpResp error, " HERE);
			break;
		}
	} while (0);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_GemCommentAlt_CreateUri(const T_CC_CMN_SMS_API_PRM *parm, Char *uri)
{
	sprintf((char*)uri,
			"%sgemcomment/alt/",
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemCommentAlt_CreateBody(const T_CC_CMN_SMS_API_PRM *parm,
								 const Char *gemId,
								 INT32 commentId,
								 const Char *comment,
								 Char *body)
{
	...
	do {
		...
		sprintf((char*)body,
				"term_id=%s&term_sig=%s&guid=%s&user_sig=%s&gem_id=%s&comment_id=%d&comment=%s",
 ...
 * @param [IN/OUT] gem      GEMコメント変更情報
 * @param [IN] opt          オプション情報
 * @param [OUT] apiStatus   APIステータス
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemCommentAlt_AnalyzeHttpResp(const Char *body,
											 ...
											 Char *apiStatus)
{
	...
	do {
		...
		ret = CC_GemCommentAlt_XmlParse((const Char*)body, &rsp_inf, opt);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemCommentAlt_XmlParse error, " HERE);
			ret = e_SC_RESULT_SMS_API_ERR;
			break;
		}
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemCommentAlt_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, SMCALOPT *opt)
{
	E_SC_RESULT		ret = e_SC_RESULT_SUCCESS;
	GEMCOMMENTALT_PARSER	gemregParser = {};
	...
	do {
		...
		XML_SetElementHandler(parser, CC_GemCommentAlt_StartElement, CC_GemCommentAlt_EndElement);
		XML_SetCharacterDataHandler(parser, CC_GemCommentAlt_CharacterData);
 ...
 */
void XMLCALL CC_GemCommentAlt_StartElement(void *userData, const char *name, const char **atts)
{
	GEMCOMMENTALT_PARSER *parser = (GEMCOMMENTALT_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
			// <xml>
			parser->state = CC_GEMCOMMENTALT_NODE_XML;
		}

		// <xml>
		if (CC_GEMCOMMENTALT_NODE_XML == parser->state) {
			if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_API_STATUS)) {
				// <api_status>
				parser->state = CC_GEMCOMMENTALT_NODE_API_STATUS;
			} else if (0 != strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
				parser->state = CC_GEMCOMMENTALT_NODE_XML_CHILD;
			}
		}
	} while (0);
 ...
 * @param [IN] name     タグ名
 */
void XMLCALL CC_GemCommentAlt_EndElement(void *userData, const char *name)
{
	GEMCOMMENTALT_PARSER *parser = (GEMCOMMENTALT_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (CC_GEMCOMMENTALT_NODE_API_STATUS == parser->state) {
			// <api_status>
			strcpy((char*)parser->gemInfo.apiStatus, (char*)parser->buf);
			parser->state = CC_GEMCOMMENTALT_NODE_XML;
		} else if (CC_GEMCOMMENTALT_NODE_XML_CHILD == parser->state) {
			parser->state = CC_GEMCOMMENTALT_NODE_XML;
		}
	} while (0);
 ...
 * @param [IN] len      データ長
 */
void XMLCALL CC_GemCommentAlt_CharacterData(void *userData, const XML_Char *data, INT32 len)
{
	GEMCOMMENTALT_PARSER *parser = (GEMCOMMENTALT_PARSER*)userData;
	//char buf[CC_CMN_XML_PARSE_DATA_SIZE + 1] = {};
	INT32	bufLen = 0;

...
int YsGenericPngDecoder::CheckSignature(char** p_Data)
{
	unsigned char buf[8];
	memcpy(buf, *p_Data, 8);
	*p_Data += 8;
...
int YsGenericPngDecoder::ReadChunk(unsigned &length,unsigned char *&buf,unsigned &chunkType,unsigned &crc,char** p_Data)
{
	unsigned char dwBuf[4];

	memcpy(dwBuf, *p_Data, 4);
	length=PngGetUnsignedInt(dwBuf);
	*p_Data += 4;
...
int YsGenericPngDecoder::Decode(FILE *fp, int Size)
{
	unsigned fileSize;
...
int YsGenericPngDecoder::Decode(char* p_Data, const int Size)
{
	unsigned fileSize;
	char* p_data_buf = p_Data;

	if(p_Data!=NULL)
	{
	    fileSize=Size;

		if(CheckSignature(&p_data_buf)!=YSOK)
		{
			printf("The file does not have PNG signature.\n");
			goto ERREND;
		}
		...
		unsigned char *buf;
		unsigned length,chunkType,crc;
		while(ReadChunk(length,buf,chunkType,crc,&p_data_buf)==YSOK && chunkType!=IEND)
		{
			switch(chunkType)
			{
			default:
